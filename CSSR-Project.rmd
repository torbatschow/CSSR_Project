...
# First results
The first model is rather inconclusive. We have choosen the simplest specification: The aggregated number of alcohol related hospitalizations in a state for three different years.
...
```
The major draw back of this regression is the low number of observations. With one observation per state the standard error are huge. Eventhough the some results are significant, the variance over different years indicates that these results are not stable.  In a second step we are looking at difference over time, which increases the number of available observations to 224:
