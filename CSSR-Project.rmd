...
1. Include Results
    Needs editing
        [major issues done]
    Result of model 5 needs to be written
        [done] First ideas 
        edit / extend
2. Add descritive stats and robustness test
    Test common trend assumption
        Question: Enough discussion of common trend assumption sofar?
4. [DONE] Rewrite literature review
    [DONE] List of what needs to be included (as comment)
    Rewrite the text and include:
        @Green.2015
        @Wilkinson.2016
        Literature recited by Marcus and Sielder
5. Layout figures
     Maps
        Position of maps (could be different on Win and Linux)
     F10.0/F10.2/K70 line chart: 
        Correct legend
        resize
     F10.0 age distribution
        resize
        color
     F10.0 time series 15.19y: 
        Change colors
        legend name 
     bar chart hypothetical F10.0 cases: 
        Label
        colors 
6. Bibliography before the appendix
    [DONE] Appendix as a separate file
    Is appendiy complete?
7. Website
    Transform presentation into website
8. Conclusion
    Write it
-->
...
<!--
Here a short paragraph on our findings. I typed a first draft.
-->
...
The advantages of this data is that they reflect a complete survey of all civilian hospitals in Germany for the time-frame of our investigation. Furthermore, the data is not self-reported by patients in interviews but instead consists of the professional diagnoses by a third party, i.e. doctors, which eliminates any problems of possible self-reporting biases. For the case study of Baden-Wuerttemberg's night sale prohibition we do also benefit from the availability of a longer time series as compared to the prior work on the issue by @Marcus.2015. Finally, the state-year aggregation level does allows us to supplement to our analysis a wide variety of other freely available data, e.g. unemployment rates etc..
...
spplot(map.Germany.2000.F100_p1000, 
       zcol = "value", 
       col.regions = colorRampPalette(rev(brewer.pal(9, "Spectral")))(22),
...
```{r echo = FALSE, out.width=c('400px')} 
# TODO: Legend label are not correct!
...
ggplot(data=filter(DS0, STATE == "DE-DE"), aes(x = YEAR, colour = Diagnoses)) +
  ...
  geom_line(aes(y = F100_p1000, colour = "F10.0")) +
  geom_line(aes(y = K70_p1000, colour = "K70", group = "cond1")) +                              # line plot
  geom_line(aes(y = F102_p1000, colour = "F10.2", group = "cond2")) +
  theme_bw() +                               # bw background
  xlab("Years") +                            
  ylab("Diagnoses \nper 1000 people") +
  ggtitle("F10.0, F10.2 and K70 diagnoses in Germany 2000-2014") +
  scale_shape_discrete(name = "Payer") +
 #scale_color_discrete(name  = "State", breaks = statelist_code, labels = statelist.code.short) +
  theme(legend.key = element_rect(size = 5, fill = 'white', colour = 'white'),
...
DS2$GROUP <- "Non-City States"
DS2$GROUP[DS2$STATE == "DE-BW"] <- "Treatment"
DS2$GROUP[DS2$STATE == "DE-BE" | DS2$STATE == "DE-HB" | DS2$STATE == "DE-HH"] <- "City"

# F10.0 cases per 1000 per age group
ggplot(data=DS2, aes(x = YEAR, y = F100_p1000, group = STATE, colour = GROUP)) +
  scale_x_continuous(breaks = c(2000, 2005, 2010, 2014), minor_breaks = c(2000:2014)) +
  geom_line() +                              # line plot
  theme_bw() +                               # bw background
  xlab("Years") +                            
...
The next step in refining our model (*Model 3*) takes us to analyzing a policy measure, namely the introduction of a ban on night sales of alcohol in the state of Baden-Wuerttemberg. Contrary to the prior analysis, we now look at the age group of 15 to 19 year olds. The results are shown in *Table 3*. While we are introducing a dummy for the state in consideration, one for the post-treatment period, and the interaction term of both, the only significant variable is the post-treatment dummy. Even more counterintuitively, the post-treatment dummy has is *positively* correlated with short-term ARHs. This hints to the fact that we might witness a common trend over time, increasing short-term ARHs, that is similar for all states and that is currently captured by the time dummy. That would explain why the time dummy (which essentially splits the whole set of observations into two consecutive phases) is highly significant but neither the state dummy nor the interaction term are of any significance.
...
stargazer::stargazer(mod3.age15.16, mod3.age15.13, mod4.age15, mod4.age15.13,
                     column.labels = c("(3) All states", "(3) non-city states",
                                       "(4) All states", "(5) non-city states"),
...
# stargazer::stargazer(mod4.age15, mod4.age15.13,  
#                     column.labels = c("All states", "non-city states"),
...
Overall, our research supports the general conclusion of the literature on drinking and socio-economic factors. In particular, the most sophisticated approach (model 5) we used finds that higher youth unemployment is correlated with more alcohol-related diagnoses amoung young people and that the night sales ban on alcohol has a measurable negative effect on binge-drinking. But due to the limitations of our data this finding is not fully consistent over the models we used. However, this inconsistency should not be overevaluated as it mostlikely steams from the limitations of the used model and the limited data available.

As our study reengenered @Marcus.2015 the confirmation of their general result deserves particular attention. While their study used more fain-grained data, our research included a longer treatment horizon. Hence, the effect @Marcus2015 find prevailed over the time horizon of their research according to our findings.

The research could be further improved by using more diaggregated data, in particular by using monthly numbers and the county level. This would increase the number of observations tremendously and allow therefore for more substantial results. It would also allow the inclusion of more controls as in we only relied on a small number of socio-economic controls. Many more controls exist, but we did not include them as they are not available as a time-series and on the regional level. 
